...
# designer/scene/*.mjs is a MECHANICAL mirror produced by
# scripts/sync-shared-scene.sh (see that script for why a mirror exists at
# all — separate Docker build contexts). This script is the enforcement side:
...
for file in "${SRC}"/*.mjs; do
  ...
  # sync-shared-scene.sh) before the byte-identical source content.
  if ! diff -q <(tail -n +4 "${mirror}") "${file}" > /dev/null 2>&1; then
