(function () {
  ...
  var PAGES = {
    fonts: {
      ...
      badge: "Типографика",
      intro: "Проверенные семейства, пары и сценарии для продуктовых интерфейсов, презентаций и рабочих документов NOFIDA.",
      notice: "Откройте семейство, скачайте файл и добавьте его в пространство команды через нативную загрузку.",
      actions: [
    ...
    media: {
      ...
      intro: "Иконки, иллюстрации и визуальные материалы для рабочих проектов. Все материалы хранятся внутри NOFIDA и готовы для рабочих сценариев.",
      notice: "Скачивайте ассеты, копируйте локальные ссылки и собирайте библиотеку команды без перехода во внешние сервисы.",
      actions: [
        { label: "Открыть экран шрифтов", href: "#/dashboard", action: "native-fonts" },
        { label: "Открыть импорт из Figma", href: PAGE_ROUTES.figma },
    ...
    figma: {
      ...
      badge: "План переноса",
      intro: "Подготовьте файл к переносу и получите понятный план миграции. Сложные компоненты, автолэйауты и прототипы отмечаются как отдельные ручные шаги.",
      notice: "Ссылка, экспорт и ассеты собираются в локальный черновик без прямого изменения файлов проекта.",
      actions: [
  ...
  var RESOURCE_CSS = [
    "#nfr-overlay,#nfr-detail-overlay{color:var(--nf-text);font-family:var(--nf-font)}",
    "#nfr-overlay[hidden],#nfr-detail-overlay[hidden],#nfr-toast[hidden]{display:none!important}",
    "#nfr-dynamic-fonts{display:none}",
    "#nfr-toast{position:fixed;left:50%;bottom:24px;transform:translateX(-50%);z-index:2147483605;padding:10px 14px;border-radius:999px;border:1px solid rgba(15,118,110,.18);background:#ffffff;color:var(--nf-text);font-size:12px;font-weight:700;box-shadow:var(--nf-shadow-md)}",
    "#nfr-shell-root{display:grid;gap:18px}",
    ".nfr-stack{display:grid;gap:18px}",
    ".nfr-actions,.nfr-pill-row,.nfr-tag-row,.nfr-card-actions,.nfr-native-tabs,.nfr-action-row,.nfr-file-list,.nfr-native-actions,.nfr-detail-actions,.nfr-status-row{display:flex;flex-wrap:wrap;gap:8px}",
    ".nfr-btn,.nfr-copy-btn,.nfr-icon-btn,.nfr-link-btn,.nfr-disabled-btn,.nfr-tab,.nfr-back{font:inherit}",
    ".nfr-btn,.nfr-link-btn,.nfr-copy-btn,.nfr-icon-btn,.nfr-back{display:inline-flex;align-items:center;justify-content:center;min-height:36px;padding:0 14px;border-radius:10px;font-size:13px;font-weight:700;text-decoration:none;cursor:pointer;transition:transform 120ms ease,box-shadow 120ms ease,background 120ms ease}",
    ".nfr-btn:hover,.nfr-link-btn:hover,.nfr-copy-btn:hover,.nfr-icon-btn:hover,.nfr-back:hover{transform:translateY(-1px)}",
    ".nfr-btn-primary{background:var(--nf-brand-strong);border:1px solid transparent;color:#fff;box-shadow:0 8px 18px rgba(6,95,70,.18)}",
    ".nfr-btn-secondary,.nfr-link-btn,.nfr-copy-btn,.nfr-icon-btn,.nfr-back,.nfr-tab{background:#fff;border:1px solid var(--nf-border);color:var(--nf-text-soft);box-shadow:var(--nf-shadow-sm)}",
    ".nfr-disabled-btn{display:inline-flex;align-items:center;justify-content:center;min-height:36px;padding:0 14px;border-radius:10px;border:1px dashed var(--nf-border-strong);background:#f8fafc;color:var(--nf-text-dim);font-size:12px;font-weight:700;cursor:not-allowed;text-decoration:none}",
    ".nfr-stat-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px}",
    ".nfr-stat,.nfr-panel,.nfr-card,.nfr-pattern-card,.nfr-native-panel,.nfr-detail-card,.nfr-report-block,.nfr-meta-block{background:var(--nf-surface);border:1px solid var(--nf-border);box-shadow:var(--nf-shadow-card)}",
    ".nfr-stat{border-radius:16px;padding:14px}",
    ".nfr-stat-value{display:block;font-size:22px;font-weight:800;line-height:1.05;color:var(--nf-text)}",
    ".nfr-stat-label{display:block;margin-top:6px;color:var(--nf-text-muted);font-size:12px;line-height:1.45}",
    ".nfr-panel,.nfr-native-panel,.nfr-detail-card{border-radius:16px;padding:16px}",
    ".nfr-panel h2,.nfr-native-panel h2,.nfr-card h3,.nfr-pattern-card h3,.nfr-detail-card h2,.nfr-report-block h3{margin:0;font-size:18px;line-height:1.25;color:var(--nf-text)}",
    ".nfr-label{margin:0 0 8px;color:var(--nf-brand);font-size:11px;font-weight:800;letter-spacing:.10em;text-transform:uppercase}",
    ".nfr-panel-copy,.nfr-detail-copy,.nfr-card-copy,.nfr-native-copy,.nfr-helper,.nfr-footer-note,.nfr-report-block p,.nfr-report-block li{color:var(--nf-text-muted);font-size:12px;line-height:1.58}",
    ".nfr-list{display:grid;gap:8px;margin:12px 0 0;padding:0;list-style:none}",
    ".nfr-list li,.nfr-empty,.nfr-error,.nfr-note,.nfr-native-empty{padding:12px 14px;border-radius:14px;border:1px solid var(--nf-border);background:var(--nf-surface-soft);font-size:13px;line-height:1.6;color:var(--nf-text-muted)}",
    ".nfr-toolbar{display:grid;grid-template-columns:minmax(0,1.4fr) repeat(4,minmax(0,170px));gap:8px;align-items:center}",
    ".nfr-filter-search,.nfr-filter-select,.nfr-textarea,.nfr-input{width:100%;min-height:38px;border-radius:11px;border:1px solid var(--nf-border);background:#fff;color:var(--nf-text);padding:0 12px;font-size:13px;box-sizing:border-box;box-shadow:var(--nf-shadow-sm);outline:none}",
    ".nfr-textarea{min-height:110px;padding:10px 12px;resize:vertical}",
    ".nfr-grid,.nfr-pattern-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px}",
    ".nfr-card,.nfr-pattern-card,.nfr-native-card{display:flex;flex-direction:column;gap:10px;border-radius:16px;padding:14px}",
    ".nfr-card-top,.nfr-native-card-top,.nfr-section-head,.nfr-native-head,.nfr-detail-topbar{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap}",
    ".nfr-pill,.nfr-tag,.nfr-native-meta span,.nfr-file-chip{display:inline-flex;align-items:center;min-height:24px;padding:0 8px;border-radius:999px;border:1px solid var(--nf-border);background:var(--nf-surface-soft);color:var(--nf-text-soft);font-size:11px;font-weight:700}",
    ".nfr-pill.good{border-color:rgba(5,150,105,.16);background:var(--nf-green-soft);color:var(--nf-green)}",
    ".nfr-pill.warn{border-color:rgba(217,119,6,.16);background:var(--nf-amber-soft);color:var(--nf-amber)}",
    ".nfr-preview,.nfr-native-preview{padding:14px 16px;border-radius:14px;border:1px solid var(--nf-border);background:linear-gradient(135deg,#eef4f8,#f9fbfd);color:var(--nf-text);font-size:20px;line-height:1.28}",
    ".nfr-meta,.nfr-detail-meta{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}",
    ".nfr-meta-block strong{display:block;font-size:10px;letter-spacing:.06em;text-transform:uppercase;color:var(--nf-text-dim)}",
    ".nfr-meta-block span{display:block;margin-top:6px;color:var(--nf-text-soft);font-size:12px;line-height:1.45}",
    ".nfr-media-thumb,.nfr-pattern-thumb,.nfr-detail-thumb{display:block;width:100%;aspect-ratio:16/10;object-fit:cover;border-radius:14px;border:1px solid var(--nf-border);background:#e2e8f0}",
    ".nfr-section-title{margin:0;font-size:16px;line-height:1.25;color:var(--nf-text)}",
    ".nfr-native-panel{margin:0 0 16px}",
    ".nfr-native-panel-compact{padding:14px 16px}",
    ".nfr-native-toolbar{display:grid;grid-template-columns:minmax(0,1fr) 180px auto;gap:8px;align-items:center}",
    ".nfr-native-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}",
    ".nfr-native-card{border:1px solid var(--nf-border);background:var(--nf-surface-soft)}",
    ".nfr-detail-shell{position:fixed;inset:0;z-index:2147483600;background:rgba(15,23,42,.22);display:flex;align-items:stretch;justify-content:flex-end;padding:20px;box-sizing:border-box}",
    ".nfr-detail-card{width:min(520px,100%);margin-left:auto;overflow-y:auto;display:flex;flex-direction:column;gap:12px}",
    ".nfr-form-grid{display:grid;grid-template-columns:minmax(0,1.1fr) minmax(0,.9fr);gap:14px}",
    ".nfr-form-panel{display:flex;flex-direction:column;gap:12px}",
    ".nfr-field{display:flex;flex-direction:column;gap:6px}",
    ".nfr-field label{color:var(--nf-text-soft);font-size:12px;font-weight:700}",
    ".nfr-toggle{display:flex;align-items:center;gap:8px;color:var(--nf-text-soft)}",
    ".nfr-toggle input{width:16px;height:16px}",
    ".nfr-dropzone{display:flex;flex-direction:column;gap:8px;align-items:flex-start;padding:14px;border-radius:14px;border:1px dashed rgba(15,118,110,.28);background:rgba(15,118,110,.05);cursor:pointer}",
    ".nfr-dropzone strong{font-size:13px;color:var(--nf-text)}",
    ".nfr-dropzone span{font-size:12px;color:var(--nf-text-muted);line-height:1.5}",
    ...
    "@media (max-width:1180px){.nfr-toolbar{grid-template-columns:minmax(0,1fr) 1fr 1fr}.nfr-grid,.nfr-pattern-grid,.nfr-native-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.nfr-form-grid{grid-template-columns:1fr}.nfr-detail-card{width:min(620px,100%)}}",
    "@media (max-width:900px){.nfr-toolbar,.nfr-native-toolbar,.nfr-meta,.nfr-detail-meta{grid-template-columns:1fr}.nfr-grid,.nfr-pattern-grid,.nfr-native-grid{grid-template-columns:1fr}.nfr-btn,.nfr-link-btn,.nfr-copy-btn,.nfr-icon-btn,.nfr-back,.nfr-disabled-btn{width:100%}}"
  ].join("");
  ...
  function renderFontBody(catalog) {
    ...
    return [
      ...
      renderNotePanel(
        "Как использовать",
        "Рабочий сценарий со шрифтами",
        "Выберите семейство, скачайте файл и загрузите его в пространство команды. Каталог помогает быстро сверить пары, сценарии и лицензии.",
        [
          "Откройте карточку семейства и проверьте подходящий сценарий использования.",
          "Скачайте файл и добавьте его через нативную загрузку команды.",
          "Соберите пару для интерфейса, документации или презентации до начала детальной сборки."
        ]
      ),
  ...
  function renderMediaBody(catalog) {
    ...
    return [
      ...
      renderNotePanel(
        ...
        [
          "Все материалы хранятся внутри NOFIDA и готовы для рабочих проектов.",
          "Карточки помогают быстро скачать ассет, открыть детали или скопировать локальную ссылку.",
          "Используйте фильтры, чтобы собрать подборку для продукта, лендинга или презентации."
        ]
      ),
  ...
  function renderFigmaBody() {
    ...
    return [
      ...
      renderStats([
        ...
        { value: "План переноса", label: "Сначала анализируются страницы, шрифты, ассеты и ручные шаги" },
        { value: "Честная оценка", label: "Сложные компоненты и прототипы выделяются как отдельные задачи" }
      ]),
      '<section class="nfr-panel">',
      '  <div class="nfr-section-head">',
      '    <div>',
      ...
      "    </div>",
      "  </div>",
      '  <p class="nfr-panel-copy">NOFIDA сначала анализирует файл и готовит план переноса. Отчет помогает команде заранее увидеть страницы, ассеты, шрифты и участки ручной доработки.</p>',
      ...
      '        <p class="nfr-helper">Поле можно оставить пустым, если у вас есть только экспорт или набор ассетов.</p>',
      "      </div>",
      '      <div class="nfr-field">',
      ...
      '        <p class="nfr-helper">Если токена пока нет, помощник все равно соберет предварительный план по ссылке, файлам и заметкам.</p>',
      "      </div>",
      '      <div class="nfr-field">',
      ...
      '        <p class="nfr-helper">Сложные компоненты, автолэйауты и прототипы обычно требуют отдельной ручной проверки.</p>',
      "      </div>",
